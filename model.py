...
def normalise_features(vector):
	...
	#changed this function. The minimal score is 3
    return ((vector - 3) / (10 - 3)).clip(min=0)
...
class NeuralNet:
    ...
    def build_model(self):
        with self.graph.as_default():
			...
			#changed the target placeholder to [None,2]
            self.target = tf.placeholder(tf.float32, shape=[None, 2], name='target')
			...
			#reduced the numer of nods to 2
            self.output = tf.layers.dense(hidden_layer2, 2, name="output")
...
if __name__ == '__main__':
    ...
    inputs = np.loadtxt('C:/SoccerPrediction/input.csv', delimiter=',')
    inputs = normalise_features(inputs)
    outputs = np.loadtxt('C:/SoccerPrediction/output.csv', delimiter=',').reshape(-1, 2)
...
# had to comment this line, since I have score = 0 (1 / 0 = error)
#     outputs = 1 / outputs

    net = NeuralNet()

    net.train_model(inputs[:-10], outputs[:-10], inputs[-10:], outputs[-10:], model_name='./saved/savedfile')

    net = NeuralNet()

    predictions = net.predict(inputs[-50:], model_name='./saved/savedfile')
